...
export function loopPokemon(e, pre){
  const names = e.results.map((item, index) => [index + 1, item.name])
  if(pre === ""){
    return names
  }else{
...
export function loopPokemon2(e, pre){
  let arr = []
  for(let i = 0; i < e.results.length; i++){
    if(pre === ""){
      arr.push([i + 1, e.results[i].name])
    }else{
      if(e.results[i].name.startsWith(pre) || (i + 1).toString().startsWith(pre)){
        arr.push([i + 1, e.results[i].name])
      }
    }
  }
...
export function getIdFromURL(e){
  let index = e.lastIndexOf("/") + 1
  let num = parseInt(e.slice(index, e.length))
  return num
}
...
export function getTypes(e){
  let types = {
    ...
    920: ['bug', 'dark'],
    921: ['electric'],
    922: ['electric', 'fighting'],
    923: ['electric', 'fighting'],
    924: ['normal'],
    925: ['normal'],
    926: ['fairy'],
    927: ['fairy'],
    ...
    1005: ['dragon', 'dark'],
    1006: ['fairy', 'fighting'],
    1007: ['fighting', 'dragon'],
    1008: ['electric', 'dragon'],
    1009: ['water', 'dragon'],
    1010: ['grass', 'psychic'],
    1011: ['grass', 'dragon'],
    1012: ['grass', 'ghost'],
